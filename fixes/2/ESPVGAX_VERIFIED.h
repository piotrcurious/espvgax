...
VERIFIED OPTIMIZATIONS & BUG FIXES (v1.0.2):
- VERIFIED: clear() using fbw (32-bit) IS CORRECT - memset operates on bytes regardless
- VERIFIED: All ESP8266 hardware macros against official esp8266_peri.h
- Added const correctness throughout
- Fixed potential overflow in rand() function
- Added nullptr checks where appropriate
- Improved bounds checking
- Fixed delay() overflow handling
- All macros match ESP8266 Arduino Core esp8266_peri.h
*/
#ifndef __ESPVGAX_LIBRARY__
#define __ESPVGAX_LIBRARY__

#define ESPVGAX_VERSION "1.0.2"
...
public:
   ...
   *    
   *    VERIFIED CORRECT: Using fbw (uint32_t*) with memset is CORRECT and
   *    EFFICIENT. memset() operates on BYTES regardless of pointer type.
   *    The compiler generates optimized code using 32-bit operations when
   *    the pointer is aligned to 4 bytes (which fbw is, via ESPVGAX_ALIGN32).
   *    Using fbw instead of fbb allows the compiler to know the alignment,
   ...
   *    Added nullptr checks for safety.
   */
   ...
   *      appear. This is because of the many iterations required by these
